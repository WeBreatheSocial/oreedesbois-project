...
export default function (req, res) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    host: "smtp.gmail.com",
    auth: {
      user: 'wbs.notifications@gmail.com',
      pass: process.env.password,
    },
    secure: true,
  })
  const mailData = {
    from: 'wbs.notifications@gmail.com',
    to: `yfouad@koutoubia.net,helferra@koutoubia.net,hallali@koutoubia.net`,
    subject: `Message de : ${req.body.name}`,
    text: `Message de : ${req.body.name} <br/>` + " | Envoyé via cet email: " + req.body.email,
    html: `<div>${req.body.message}</div><p>Envoyé via cet email:
    ${req.body.email}</p><p>Téléphone: ${req.body.phone}</p>`
  }
